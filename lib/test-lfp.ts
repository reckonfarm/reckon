...
const TEST_COUNTIES = [
  { fips: '48011', label: 'Armstrong TX',  expectedTier: '6 (D4 ≥4 weeks → 5 payments)' },
  { fips: '31003', label: 'Antelope NE',   expectedTier: '3 or 4 (D3 tier)'              },
  { fips: '30069', label: 'Petroleum MT',  expectedTier: '1 (D2 ≥4 consec, no D3/D4)'   },
] as const
...
async function main() {
  ...
  for (const { fips, label, expectedTier } of TEST_COUNTIES) {
    const bar = '─'.repeat(64)
    console.log(bar)
