...
import { membershipRole, setMemberRole, isRole } from '@/lib/invitations'

// POST /api/members/role { user_id, role } → { ok }   OWNERS ONLY; never demotes the last owner.
export async function POST(req: NextRequest) {
  ...
  if ((await membershipRole(s.supabase, s.user.id, ranchId)) !== 'owner') return NextResponse.json({ error: 'Only an owner can change roles.' }, { status: 403 })
  const r = await setMemberRole(createServiceClient(), ranchId, target, body.role)
