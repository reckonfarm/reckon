...
import { correctEvent } from '@/lib/corrections'

// ─── POST /api/activity/[id]/correct (Block 5B) ────────────────────────────────
//   body { id?: uuid (client-minted, retried with the same id), reason?: text, ...the fields that changed (bales, inches, ts, place_id, herd_lot_id, …) }
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const r = await correctEvent(s.supabase, s.user.id, id, (body ?? {}) as Record<string, unknown>)
