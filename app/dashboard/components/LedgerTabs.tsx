...
export function LedgerLoading() {
  return <p className="px-1 py-3 font-dm-sans text-sm text-ink">Adding it up…</p>
}
      ...
      <div>
        ...
        {TABS.map(t => (
          <div key={t} id={`ledger-${t}`} role="tabpanel" hidden={t !== active} className="mt-3">
            {empty[t] && (
              <p className="rounded-xl border border-dashed border-forest-green/20 px-4 py-5 text-center font-dm-sans text-sm text-secondary-ink">
                {EMPTY[t]}
              </p>
            )}
            {bodies[t]}
