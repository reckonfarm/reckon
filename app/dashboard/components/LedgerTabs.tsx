      ...
      <div>
          ...
          {TABS.map(t => {
            ...
            return (
              <button
                ...
                onClick={() => setActive(t)}
                className={`min-h-[48px] flex-1 rounded-lg px-2 font-dm-sans text-[16px] transition-colors ${
        ...
        {TABS.map(t => (
          <div key={t} id={`ledger-${t}`} role="tabpanel" hidden={t !== active} className="mt-3">
            {empty[t] && (
              <p className="rounded-xl border border-dashed border-forest-green/20 px-4 py-5 text-center font-dm-sans text-[16px] text-secondary-ink">
                {EMPTY[t]}
              </p>
            )}
            {bodies[t]}
