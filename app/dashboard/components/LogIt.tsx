...
function Counter({ label, value, onChange, unit, audit, min = 0, max = 20000 }: { label: string; value: string; onChange: (v: string) => void; unit?: string; audit: string; min?: number; max?: number }) {
  ...
  return (
    <div data-audit={audit}>
      <p className="font-dm-sans text-[16px] font-medium text-ink">{label}</p>
      <div className="mt-1 flex items-center gap-2">
        <button type="button" onClick={() => set(n - 1)} disabled={n <= min} className={btn} aria-label={`${label} minus one`} data-audit={`${audit}-minus`}>−</button>
        ...
        <button type="button" onClick={() => set(n + 1)} className={btn} aria-label={`${label} plus one`} data-audit={`${audit}-plus`}>+</button>
