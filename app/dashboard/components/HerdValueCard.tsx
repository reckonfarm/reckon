...
export default function HerdValueCard({ anchor }: { anchor: HerdAnchor }) {
  ...
  return (
    ...
    <Card shadow="none" className="px-5 py-4" data-audit="herd-value-card">
        ...
        {priced ? (
          <>
            <p className="mt-1.5 type-main-number text-ink">
            ...
            {sensitivity && <p className="mt-1 font-dm-sans text-[16px] font-medium text-forest-green">{sensitivity}</p>}
            {sources.length > 0 && (
              <p className="mt-1 font-dm-sans text-[16px] text-ink">
          ...
          <p className="mt-1.5 font-dm-sans text-[16px] text-ink">{estimate.note}</p>
        )}
        {minFloor != null && (
          <p className="mt-1 font-dm-sans text-[16px] text-ink">
