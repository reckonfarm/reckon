...
export default async function MarketsHistory({ resolved, lots, selectedLotId = null, mode = 'cattle' }: { resolved: ResolveResult; lots: Lot[]; selectedLotId?: string | null; mode?: 'cattle' | 'context' }) {
  ...
  // Block 14: a bunch with no weight set cannot drive a weight band; the chart takes the next feeder bunch that has one.
  const hasWeight = (l: Lot) => l.avg_weight != null
  const feederLot = (selectedLotId ? lots.find(l => l.id === selectedLotId && isFeeder(l) && hasWeight(l)) : null) ?? lots.find(l => isFeeder(l) && hasWeight(l)) ?? null
  ...
  const lot = feederLot ? (() => { const w = lotToMarsKey(feederLot).avgWeightLb ?? 0; return { head: feederLot.head_count, weightLb: w, label: `${lotLabel(feederLot)} · ${feederLot.head_count} head`, cls: (feederLot.class === 'heifers' ? 'Heifers' : 'Steers') as 'Steers' | 'Heifers', band: String(Math.max(300, Math.min(900, Math.floor(w / 100) * 100))) } })() : null
