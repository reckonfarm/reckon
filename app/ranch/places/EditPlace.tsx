...
type Mode = 'idle' | 'editing' | 'confirmRetire'
    ...
    return (
      <Card className="mt-4 p-4 sm:p-5" data-audit="place-confirm-retire">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" disabled={busy} onClick={() => send({}, 'DELETE')} className="min-h-[52px] flex-1 rounded-lg bg-forest-green px-4 font-dm-sans text-[17px] font-semibold text-cream disabled:opacity-50" data-audit="place-retire-confirm">
  ...
  if (mode === 'editing') {
    return (
      <Card className="mt-4 p-4 sm:p-5" data-audit="place-edit">
        ...
        <button type="button" disabled={busy} onClick={() => { setError(null); setMode('confirmRetire') }} className="mt-4 inline-flex min-h-[44px] items-center font-dm-sans text-[16px] font-semibold text-secondary-ink underline underline-offset-2 disabled:opacity-50" data-audit="place-retire-open">
          Retire this place
        </button>
      </Card>
    )
