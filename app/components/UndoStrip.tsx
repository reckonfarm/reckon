...
export default function UndoStrip() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 z-40 px-4" style={{ bottom: 'calc(env(safe-area-inset-bottom, 0px) + 64px)' }} data-audit="undo-layer">
      <div className="pointer-events-auto mx-auto max-w-2xl overflow-hidden rounded-lg bg-ink text-cream shadow-lg" role="status" aria-live="polite" data-audit="undo-strip" data-state={item.state} onClick={item.state !== 'shown' && item.state !== 'undoing' ? dismissUndo : undefined}>
