...
// "Billings · Sep 3 · Reference sale: 42 head · Report ↗" (5F wording). One shared component, so no price
...
export default function ReportEvidence({ barn, date, head, slug, revision, className = '' }: {
  ...
  return (
    <span className={`inline-flex flex-wrap items-baseline gap-x-1 ${className}`} data-audit="report-evidence">
      <span>{barn} · {fmtShort(date)}{head != null ? ` · Reference sale: ${head.toLocaleString('en-US')} head` : ''}{revision && revision > 1 ? ` · rev ${revision}` : ''} ·</span>
