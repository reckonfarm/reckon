...
async function main() {
  ...
  try {
    ...
    await section('Block 6 (6K): weather copy — no "County County"; the rainfall line says what it is; the Drought Monitor\'s two dates named apart', async () => {
      ...
      // The map is a lazy loader (Leaflet arrives after the page): wait for its paint before counting — the 44 check
      // reads later in the page's life and finds it; read too early this reported 0 on a page that had it.
      const mapSel = '[data-audit="weather-drought-map"] .leaflet-container, [data-audit="weather-drought-map"] img'
      await page.locator(mapSel).first().waitFor({ timeout: 20_000 }).catch(() => {})
      const mapOpen = await page.locator(mapSel).count()
