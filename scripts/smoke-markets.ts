...
async function main() {
  ...
  try {
    ...
    await page.getByText('Cattle markets · history').waitFor({ timeout: 30_000 }).catch(() => {})
    ...
    record('A5: culls listed as slaughter prices, not breeding value', /Culls · slaughter prices, not breeding value/i.test(body) && /(Breaker|Boner|Lean|Cull cows|Slaughter bulls)/.test(body))
    record('B3: history card with the carried-forward toggle', /Cattle markets · history/.test(body) && /carried-forward steps/.test(body))
