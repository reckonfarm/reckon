...
async function seed() {
  ...
  await admin.from('operation_profiles').insert({ user_id: userId, ranch_id: ranch!.id, county_fips: '30069' })
  await admin.from('herd_lots').insert({ ranch_id: ranch!.id, class: 'steers', head_count: 300, avg_weight: 550, weight_unit: 'lb', created_by: userId, updated_by: userId })
}
