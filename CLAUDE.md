...
- **A check proves what it is looking at before it reads anything from it.** Assert identity first — this is the page,
  ...
  and never a pass.
- **An identity line nobody reads is not a check.** Every suite and harness prints the commit and BASE on its SUMMARY
  line, beside the counts (`suiteIdentity()` in `scripts/lib/suite-guard.ts`) — the header scrolls away, the summary is
  what gets quoted. **Counts reported without the commit they came from are a partial, not a result** — in the run's
  own output and in the report to PK.
